...
- (void)layoutSubviews
{
    ...
    if (self.contentView != nil) {
        self.contentView.frame = CGRectMake(x, 20, self.sideBarWidth, self.parentViewController.view.bounds.size.height);
    }
}
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated
{
    ...
    sideBarFrame.origin.y=self.showFromRight ? self.view.bounds.size.height:  20;
    void (^animations)() = ^{
        if (self.contentView != nil) {
            self.contentView.frame = sideBarFrame;
        }
...
- (void)startShow:(CGFloat)startX
{
    ...
    CGRect sideBarFrame = self.view.bounds;
    sideBarFrame.origin.x = self.showFromRight ? parentWidth : -self.sideBarWidth;
    sideBarFrame.origin.y = self.showFromRight ? parentWidth : 20.f;
    sideBarFrame.size.width = self.sideBarWidth;
    if (self.contentView != nil) {
        self.contentView.frame = sideBarFrame;
    }
